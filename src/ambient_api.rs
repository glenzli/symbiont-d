...
fn sensing_instructions() -> String {
    format!(
        "Run one low-cost ambient sensing pass for the supplied independent input role. Follow its focus, but do not pretend it defines the user's interests. Use web search for credible fresh or recently active external developments with information or discussion value. Standalone science, mathematics, culture, public events, products, and unusual real-world phenomena are valid without a project connection; do not spend this pass proving user relevance. Later independent evaluation, adoption, failure, accumulated reaction, or a clearer interpretive tension can make an older event timely again. Prefer primary or authoritative sources for facts and credible independent sources for reception and reproducibility. This is intake only: do not create memory, alter context, or write user-visible prose. When search yields a defensible concrete signal, default to submitting it for independent review instead of silently applying the conversational assistant's relevance bar. Submit nothing only when search or tooling produced no defensible signal. Call submit_sensing_candidates at most once with one to three candidates and concrete sources. \n\n{}",
        crate::sensing::MODEL_INPUT_WRITING_CONTRACT
    )
}
fn sensing_tool_schema() -> Value {
    json!({"type":"object","properties":{"candidates":{"type":"array","minItems":1,"maxItems":3,"items":{"type":"object","properties":{"title":{"type":"string","maxLength":240},"summary":{"type":"string","maxLength":1000},"proposed_input":{"type":"string","maxLength":1800,"description":crate::sensing::MODEL_INPUT_DESCRIPTION},"event_at":{"type":"string","maxLength":64},"source_class":{"type":"string","enum":["research","products_and_tools","projects_and_ecosystems","institutions_and_policy","industry_and_markets","culture_and_ideas","open_discovery"]},"possible_connection":{"type":"string","maxLength":800},"sources":{"type":"array","minItems":1,"maxItems":3,"items":{"type":"object","properties":{"url":{"type":"string","maxLength":900},"detail":{"type":"string","maxLength":800,"description":crate::sensing::MODEL_SOURCE_DESCRIPTION}},"required":["url","detail"],"additionalProperties":false}}},"required":["title","summary","proposed_input","source_class","sources"],"additionalProperties":false}}},"required":["candidates"],"additionalProperties":false})
}
