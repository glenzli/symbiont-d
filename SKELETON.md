...
- `src/continuity/recall_selection.rs`: foreground-only joint relevance admission for PCP, raw chat and individually gated Orientation segments through local Infer reranking, conservative lexical fallback, identity-bound source coverage, and diagnostic-only selection evidence. Ordinary chat never preloads the full Orientation; background work can still use it. Stored content and background recurrence remain independent.
...
- `src/codex/tool_surface.rs`: minimal stage registration, exact-schema discovery, deferred call normalization and origin allowlists; execution remains behind domain/evidence guards. Progress-bearing interactive, reflection, autonomous and continuation stages can read and update bounded activity snapshots, while maintenance/scout/sensing cannot; stable topic keys, unchanged-content suppression and anti-polling guidance keep activity operational rather than a transcript. It consumes the host-owned catalog, not the PCP MCP catalog.
- `src/x_browser.rs` and `scripts/x-browser-inspector.mjs`: exact X post URL admission, Host network approval, and one-shot isolated read-only Chrome inspection. Rendered text and a transient screenshot return to the model; screenshots are omitted from durable tool traces.
...
- `src/continuity/retention/experience.rs`: optional structured evidence at existing eligible retention review/publication checkpoints, with exact native receipts and an identity/principal-bound private SDK outbox. `SYMBIONT_PCP_EXPERIENCE_MEMORY=true` opts the host in; Runtime candidate policy and Scope ACL still apply. One delivery per retention call, no new model or polling loop. `/api/retention` exposes pending delivery, capacity/age failures and durable inspection pauses; after correcting a pause, the operator can clear that binding's `paused` flag in the local retention archive while the host is stopped.
...
- `src/inference/sensing_duplicate.rs`: deterministic delivery identity, bounded local `text.deduplicate` verdict and at-most-two-per-pass, at-most-three-reference Luna `text.deduplicate.review` escalation for ambiguous repeats. The escalation is stateless and receives no PCP, profile, chat history or tools; new evidence and changed results remain eligible.
...
- `web/settings-session.js`: per-page unsaved drafts, native validation and serialized save lifecycle. `web/settings.js` retains domain configuration and persistence; Reflection exposes its save operation through the same lifecycle.
